...
with st.sidebar:
    ...
    benchmark_choice = st.selectbox(
        "Select Index",
        options=["Nifty 50", "Sensex"]
    )

# --- Data Validation & Ingestion ---
if not raw_csv_content.strip():
    st.info("👈 Please upload a CSV file or paste raw CSV data in the sidebar to generate the tearsheet.")
    st.stop()
...
if returns is None or returns.empty or len(returns) < 2:
    st.error("Insufficient data points. Please supply at least 2 valid historical price entries.")
    st.stop()
...
if returns is not None and not returns.empty:
    ...
    st.caption(
        f"Analysis Period: **{start_date}** to **{end_date}** | Total Strategy Trading Days: **{len(returns)}**"
    )
    ...
    if enable_benchmark:
        with st.spinner(f"Fetching historical market data for {benchmark_choice}..."):
            raw_bench = fetch_benchmark_returns(start_date, end_date, benchmark_choice)
            if not raw_bench.empty:
                strat_aligned, bench_returns = align_strategy_and_benchmark(returns, raw_bench)
    ...
    if enable_benchmark and bench_returns is not None:
        ...
        kpi1.metric(label="Cumulative Return", value=f"{summary['Cumulative Return'] * 100:.2f}%")
        ...
        kpi5.metric(label="Max DD", value=f"{summary['Max Drawdown'] * 100:.2f}%")
        ...
        kpi5.metric(label="Max Drawdown", value=f"{summary['Max Drawdown'] * 100:.2f}%")
    ...
    table_rows = [
        ("Annualized Volatility", f"{summary['Annual Volatility'] * 100:.2f}%", "Overall standard deviation scaled to 252 trading days"),
        ("Downside Volatility", f"{summary['Downside Volatility'] * 100:.2f}%", "Annualized dispersion considering only negative return days (MAR = 0)"),
        ("Historical Daily VaR (95%)", f"{summary['Daily VaR (95%)'] * 100:.2f}%", "Maximum expected daily loss on 95% of trading days"),
        ("Historical Daily CVaR / Expected Shortfall (95%)", f"{summary['Daily CVaR (95%)'] * 100:.2f}%", "Average loss magnitude incurred on days breaching the VaR threshold"),
    ]

    if enable_benchmark and bench_returns is not None:
        table_rows.extend([
            (f"Beta (vs {benchmark_choice})", f"{beta_val:.2f}", "Sensitivity of strategy returns relative to benchmark index movements"),
            (f"Jensen's Alpha (vs {benchmark_choice})", f"{alpha_val * 100:.2f}%", "Annualized excess return generated over benchmark-implied return"),
            (f"Tracking Error (vs {benchmark_choice})", f"{te_val * 100:.2f}%", "Annualized standard deviation of daily excess returns"),
            (f"Information Ratio (vs {benchmark_choice})", f"{ir_val:.2f}", "Mean active excess return generated per unit of active risk (Tracking Error)"),
        ])

    df_metrics = pd.DataFrame(table_rows, columns=["Metric Name", "Value", "Interpretation"])
